...
function setup() {
  ...
  noFill();
  strokeWeight(4); // Set thick line weight
  
  numbers = reverse(numbers); // Reverse the array
  
  for (var i = 0; i < numbers.length - 1; i++) {
    var n = numbers[i];
    var x1 = map(i, 0, numbers.length - 1, 0, width);
    var y1 = map(n, 0, max(numbers), height, 0);
    var x2 = map(i + 1, 0, numbers.length - 1, 0, width);
    var y2 = map(numbers[i + 1], 0, max(numbers), height, 0);
    
    // Use bezier curves for a more advanced design
    var cx1 = x1 + (x2 - x1) / 2;
    var cy1 = y1;
    var cx2 = x1 + (x2 - x1) / 2;
    var cy2 = y2;
    
    // Alternate between black, red, and yellow for the stroke color
    if (i % 3 === 0) {
      stroke(0); // Black
    } else if (i % 3 === 1) {
      stroke(255, 0, 0); // Red
...
function draw() {
  // Leave draw() empty if no continuous drawing is needed
}
